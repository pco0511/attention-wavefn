...
class FinalBlock(eqx.Module):
    ...
    def __call__(
        ...
        attention_output = self.attention_block(inputs)
        output = jax.vmap(self.ff_layer)(attention_output)
        output_r = output[:, :self.n_particles]
        output_i = output[:, self.n_particles:]
        output_complex = jax.lax.complex(output_r, output_i)
...
class AWFN(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        embedder_key, attention_key, mlp_key = jax.random.split(key, 3)
    ...
    def __call__(
        ...
        output = self.final_block(embedded)
